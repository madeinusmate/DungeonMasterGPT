...
export const initialMessages: ChatGPTMessage[] = [
  {
    role: "assistant",
    content: "Are you ready to start the adventure?",
  },
];
...
const InputMessage = ({ input, setInput, sendMessage }: any) => (
  <div className="mt-6 flex clear-both">
    <input
      ...
      required
      className="min-w-0 flex-auto appearance-none rounded-md border border-zinc-900/10 bg-white px-3 py-[calc(theme(spacing.2)-1px)] shadow-md shadow-zinc-800/5 placeholder:text-zinc-400 focus:border-red-500 focus:outline-none focus:ring-4 focus:ring-red-500/10 sm:text-sm"
      value={input}
      onKeyDown={(e) => {
        if (e.key === "Enter") {
          sendMessage(input);
    ...
    <Button
      type="submit"
      className="ml-4 flex-none bg-red-600"
      onClick={() => {
        sendMessage(input);
        setInput("");
      }}
    ...
    <div className="rounded-2xl border-zinc-100  lg:border lg:p-6">
      ...
      {messages.length < 2 && (
        <span className="mx-auto flex flex-grow text-white clear-both">
          Type your adventure instructions to start
        </span>
      )}
      <InputMessage
        input={input}
